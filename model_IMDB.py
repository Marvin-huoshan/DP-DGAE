...
import os
import numpy as np
from preprocessing_Cora import GPU_data_normal_2d
import args_IMDB
...
class VGAE(nn.Module):
    def __init__(self, adj):
        ...
        # 定义卷积层1(1443 x 32)
        self.base_gcn = GraphConvSparse(args_IMDB.input_dim, args_IMDB.hidden1_dim, adj)
        # 定义卷积层2(32 x 16) 均值
        self.gcn_mean = GraphConvSparse(args_IMDB.hidden1_dim, args_IMDB.hidden2_dim, adj, activation=lambda x: x)
        # 定义卷积层3(32 x 16) log(方差)
        self.gcn_logstddev = GraphConvSparse(args_IMDB.hidden1_dim, args_IMDB.hidden2_dim, adj, activation=lambda x: x)
    ...
    def encode(self, X):
        ...
        laplace_noise = laplace.sample([X.size(0), args_IMDB.hidden2_dim])
...
class GraphConvSparse(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, adj, file, activation=F.relu, **kwargs):
        ...
        if file != 'None':
            ...
            weight = glorot_init(input_dim, output_dim)
        self.weight = weight
        self.adj = adj
...
class GAE(nn.Module):
    def __init__(self, adj):
        ...
        self.base_gcn = GraphConvSparse(args_IMDB.input_dim, args_IMDB.hidden1_dim, adj, 'None',
                                        activation=F.relu)
        # gcn_mean = GraphConvSparse(32,16,adj)
        self.gcn_mean = GraphConvSparse(args_IMDB.hidden1_dim, args_IMDB.hidden2_dim, adj, 'None',
                                        activation=F.relu)
        self.gcn_out1 = GraphConvSparse(args_IMDB.hidden2_dim, args_IMDB.extend1_dim, adj, 'None', activation=F.relu)
        self.gcn_out2 = GraphConvSparse(args_IMDB.extend1_dim, args_IMDB.extend2_dim, adj, 'None', activation=F.relu)
        self.gcn_out = GraphConvSparse(args_IMDB.extend2_dim, args_IMDB.num, adj, 'None', activation=torch.sigmoid)
    ...
    def encode(self, X):
        ...
        # hidden维度为：n x D
        hidden = self.base_gcn(X)
        hidden = GPU_data_normal_2d(hidden)
        # hidden = self.gcn_mean(hidden)
        # z的维度为n x hidden2_dim
        z = self.mean = self.gcn_mean(hidden)
        z = GPU_data_normal_2d(z)
        return z
