...
edge_file = 'data/IMDB/IMDB_edge_unweight.txt'
feature_file = 'data/IMDB/IMDB_feature_un.txt'
#edge_file = 'data/gplus/100668989009254813743.edges'
#feature_file = 'data/gplus/100668989009254813743.feat'
#获得数据集的邻接矩阵与特征矩阵
#daj = preprocess_txt_edge(edge_file)
#features = preprocess_txt_feature(feature_file)
...
Coefficient3 = torch.reshape(Cof3,(17577,17577))
...
def MTL(loss, Floss, MTLoss):
    ...
    theta1 = theta1.reshape(1,308950929)
    theta2 = theta2.reshape(1,308950929)
...
for epoch in range(args.num_epoch):
    ...
    print('pred_max_IMDB:',torch.max(A_pred))
    #梯度清零
    optimizer.zero_grad()
    #norm = (n * n) / [(n * n - num(edges)) * 2]
...
torch.save(obj=adj_label, f = 'pre_matrix/A_p_MTL_train_IMDB.pth')
torch.save(obj=A_pred, f = 'pre_matrix/A_p_MTL_IMDB.pth')

test_roc, test_ap = get_scores(test_edges, test_edges_false, A_pred)
f = open('log/log-MTL_IMDB.txt', 'w')
print('MTL_IMDB')
print('Loss_&_ACC_H3_MTL_IMDB',file = f)
...
def plot_loss_with_acc(loss_history,Floss_history,Loss_history,acc_history,roc_history,ap_history):
    ...
    plt.savefig('Loss_&_ACC_H3_MTL_IMDB.png')
